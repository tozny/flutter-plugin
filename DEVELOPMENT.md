...
Adding support for different platforms may be necessary at some point during the project's development. Below is an example of adding support for iOS to the Tozny Flutter plugin. Run this command at the top level directory of the repository. Tozny plugin has partial support for iOS, so the below command needs not be run.  
...
The `flutter create` command will complain if the repository name does not adhere to this convention, so you may need to rename your project's name. 
...
The Tozny iOS plugin requires XCode and Cocoapods installed in order to be developed. 

Before opening the project, make sure that you install all necessary pods by running the `pod install` command. This installs the dependencies and pods found in the project's Podfile and each module's `.podspec` file. 

Run `pod update` after the first time you do `pod install`. This is not necessary each time as it only updates pods in the Podfile to the latest version and pre-loads some Pods (such as E3db Swift) into the project's workspace. 

Now, before opening the project in XCode build the project by running `flutter build --release --no-codesign` in the `example/ios/` directory. When the build finishes, open the `Runner.xcworkspace` file in the `example/ios/` directory in XCode. 

### Common Issues

Most common issues that might arise when developing the iOS plugin can be resolved by one of the following steps:

* If XCode complains that modules are not able to be found, close XCode and run `pod install` in the `example/ios/` directory. This command installs the pods in the project's PodFile, and must be run every time there are added or removed pods from the PodFile. XCode must be restarted each time this is down in order to reflect the new changes in the workspace. 
* Missing modules may also need to be added to the `plugin_tozny.podspec` file. Similarly to the steps above, once changes are made here you should run `pod install` and restart XCode. 
* Make sure that when opening the project in XCode, it is the `Runner.xcworkspace` file that is opened and not a `.xcodeproj` file. 

### XCode (how to open project, when resets are necessary, etc)

XCode docs can be referenced here: ....

### Cocoapods Warnings

`pod install` and `pod update` may give warnings that 
